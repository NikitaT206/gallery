      ...
      <div onClick={(event) => event.stopPropagation()} className={styles.panel}>
        ...
        <div className={styles.panelWrapper}>
          ...
          <div className={styles.imageDescr}>
            <img className={styles.imageThumb} src={API + currentCard.thumbnailSmall}></img>
            ...
            <div className={styles.buttonsContainer}>
              <div className={styles.deleteButtonCOntainer}>
                <div className={confirmButtonsShow ? styles.confirmButtons : styles.confirmButtonsHide}>
                  <button className={styles.confirmButton} onClick={deleteImage}></button>
                  <button className={styles.cancelButton} onClick={() => setConfirmButtonsShow(false)}></button>
                </div>
                <button onClick={() => setConfirmButtonsShow(!confirmButtonsShow)} className={styles.deleteButton}></button>
              </div>
              <a href={API + currentCard.fullImage} className={styles.linkButton} download={'currentCard.fullImage'} target={'_blank'}></a>
            </div>
          </div>
         
          <p className={styles.charItemFirst}>Upload at: <span className={styles.charDesc}>{currentCard.uploadDate}</span></p>
          {currentCard.exifData && currentCard.exifData.exif && (
            <ul className={styles.charList}>
              {currentCard.exifData.image.Make && currentCard.exifData.image.Model && (
                <li className={styles.charItem}>Model: <span className={styles.charDesc}>{currentCard.exifData.image.Make + ' ' + currentCard.exifData.image.Model}</span></li>
              )}
              {currentCard.exifData.exif.DateTimeOriginal && (
                <li className={styles.charItem}>Filmed at: <span className={styles.charDesc}>{currentCard.exifData.exif.DateTimeOriginal}</span></li>
              )}
              {currentCard.exifData.exif.ISO && (
                <li className={styles.charItem}>ISO: <span className={styles.charDesc}>{currentCard.exifData.exif.ISO}</span></li>
              )}

              {currentCard.exifData.exif.FocalLength && (
                <li className={styles.charItem}>Focal Length: <span className={styles.charDesc}>{currentCard.exifData.exif.FocalLength}</span></li>
              )}
              
              {currentCard.exifData.exif.ExposureTime && (
                <li className={styles.charItem}>Exposure Time: <span className={styles.charDesc}>{currentCard.exifData.exif.ExposureTime}</span></li>
              )}
              
              {currentCard.exifData.exif.ApertureValue && (
                <li className={styles.charItem}>Aperture Value: <span className={styles.charDesc}>{currentCard.exifData.exif.ApertureValue}</span></li>
              )}
